-- =============================================
-- FIX ANALYTICS SAMPLE DATA - HANDLES DUPLICATES
-- This script clears existing data and repopulates safely
-- =============================================

-- STEP 1: Clear existing sample data (optional but recommended)
...
-- STEP 2: Insert sample data with conflict handling

-- Insert visitor stats for the last 30 days (using ON CONFLICT DO NOTHING)
...
-- Insert revenue tracking data (with unique transaction IDs)
...
(CURRENT_DATE, 'TXN_ANALYTICS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_001', 'customer1@example.com', 'Complete Python Programming Masterclass', 4999.00, 'INR', 'credit_card', 'google', 48, 0.00),
(CURRENT_DATE, 'TXN_ANALYTICS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_002', 'customer2@example.com', 'Machine Learning & AI Fundamentals', 6999.00, 'INR', 'upi', 'facebook', 72, 1000.00),
(CURRENT_DATE - INTERVAL '1 day', 'TXN_ANALYTICS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_003', 'customer3@example.com', 'Full Stack Web Development Bootcamp', 8999.00, 'INR', 'net_banking', 'direct', 24, 0.00),
(CURRENT_DATE - INTERVAL '2 days', 'TXN_ANALYTICS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_004', 'customer4@example.com', 'Data Science with R & Python', 7499.00, 'INR', 'debit_card', 'google_ads', 120, 500.00);

-- Insert real-time visitors (with unique session IDs)
...
('LIVE_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_001', '192.168.1.100', '/courses', CURRENT_TIMESTAMP - INTERVAL '5 minutes', CURRENT_TIMESTAMP - INTERVAL '1 minute', true, 'anonymous', 'google', 'desktop', 'India'),
('LIVE_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_002', '192.168.1.101', '/course/1', CURRENT_TIMESTAMP - INTERVAL '10 minutes', CURRENT_TIMESTAMP - INTERVAL '2 minutes', true, 'logged_in', 'direct', 'mobile', 'United States'),
('LIVE_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_003', '192.168.1.102', '/analytics-dashboard', CURRENT_TIMESTAMP - INTERVAL '15 minutes', CURRENT_TIMESTAMP - INTERVAL '30 seconds', true, 'paid', 'direct', 'desktop', 'India'),
('LIVE_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_004', '192.168.1.103', '/course/2', CURRENT_TIMESTAMP - INTERVAL '8 minutes', CURRENT_TIMESTAMP - INTERVAL '3 minutes', true, 'anonymous', 'facebook', 'tablet', 'United Kingdom'),
('LIVE_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_005', '192.168.1.104', '/members-login', CURRENT_TIMESTAMP - INTERVAL '3 minutes', CURRENT_TIMESTAMP - INTERVAL '1 minute', true, 'anonymous', 'instagram', 'mobile', 'Canada');
...
-- Insert sample session tracking
...
('SESS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_001', '192.168.1.100', 'Mozilla/5.0 (Windows NT 10.0; Win64; x64)', CURRENT_TIMESTAMP - INTERVAL '2 hours', CURRENT_TIMESTAMP - INTERVAL '1 hour 45 minutes', 900, 5, ARRAY['/courses', '/course/1', '/course/2', '/about', '/contact'], 12, 85, false, '/contact', 'enquiry'),
('SESS_' || EXTRACT(EPOCH FROM CURRENT_TIMESTAMP)::BIGINT || '_002', '192.168.1.101', 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_15_7)', CURRENT_TIMESTAMP - INTERVAL '3 hours', CURRENT_TIMESTAMP - INTERVAL '2 hours 50 minutes', 600, 3, ARRAY['/courses', '/course/1', '/members-login'], 8, 70, false, '/members-login', 'login');
...
-- Insert quiz performance data (using timestamp to avoid conflicts)
...
DO $$
BEGIN
    RAISE NOTICE '🎉 ANALYTICS SAMPLE DATA FIXED AND POPULATED!';
    RAISE NOTICE '✅ All duplicate key conflicts resolved';
    RAISE NOTICE '📊 Sample data ready for Analytics Dashboard';
    RAISE NOTICE '🚀 Navigate to /analytics-dashboard to test';
END $$; 